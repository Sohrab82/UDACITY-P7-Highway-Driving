...
// Initializes GNB
GNB::GNB(int num_features)
{
    /**
   * TODO: Initialize GNB, if necessary. May depend on your implementation.
   */
    num_features_ = num_features;
    left_means = ArrayXd(num_features_);

    left_sds = ArrayXd(num_features_);

    left_prior = 0;

    keep_means = ArrayXd(num_features_);

    keep_sds = ArrayXd(num_features_);

    keep_prior = 0;

    right_means = ArrayXd(num_features_);

    right_sds = ArrayXd(num_features_);
